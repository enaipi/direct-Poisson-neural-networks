...
def simulate_fpu_trajectories():
    ...
    save_standard_json(trajectories, data_dir)
    save_transition_csv(trajectories, data_dir)
...
def save_standard_json(trajectories, data_dir):
    """Save trajectories in the standard JSON format."""
    data = {
        "metadata": {
            "system_name": "FermiPastaUlam",
            "dimension": 2 * STATE_DIMENSIONS,
            "dt": DT,
            "num_trajectories": len(trajectories),
            "trajectory_length": STEPS_PER_TRAJECTORY,
        ...
        "trajectories": [
            {
                ...
                "metadata": {"state_order": "q_then_p"},
            }
            for trajectory_id, trajectory in enumerate(trajectories)
        ],
    }

    with open(data_dir / "fpu_data.json", "w") as f:
        json.dump(data, f, indent=2)


def save_transition_csv(trajectories, data_dir):
    ...
    for trajectory_id, trajectory in enumerate(trajectories):
        for step in range(len(trajectory) - 1):
            ...
            for idx in range(STATE_DIMENSIONS):
                row[f"old_q{idx}"] = old_state[idx]
                row[f"q{idx}"] = new_state[idx]
            for idx in range(STATE_DIMENSIONS):
                p_idx = STATE_DIMENSIONS + idx
                row[f"old_p{idx}"] = old_state[p_idx]
                row[f"p{idx}"] = new_state[p_idx]
            rows.append(row)

    import pandas as pd

    pd.DataFrame(rows).to_csv(data_dir / "fpu_transitions.csv", index=False)
