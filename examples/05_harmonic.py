...
from dpnn.utils._common import (
    TransitionDataset,
    save_transition_csv,
    save_standard_json,
)
...
def simulate_harmonic_trajectories():
    ...
    component_labels = [
        f"{n}_{d}" for n in range(N_PARTICLES) for d in range(DIMENSIONS)
    ]

    save_standard_json(
        trajectories,
        data_dir,
        filename="harmonic_data.json",
        system_name="HarmonicOscillatorParticles",
        dt=DT,
    ...
    save_transition_csv(
        trajectories,
        data_dir,
        filename="harmonic_transitions.csv",
        dt=DT,
        component_dim=STATE_DIMENSIONS,
        component_labels=component_labels,
    )

    energy_array = np.vstack(energies)
    relative_energy_drift = np.max(
        np.abs(energy_array - energy_array[:, :1])
        / np.maximum(np.abs(energy_array[:, :1]), 1e-12)
    )
...
def inspect_learned_model(learner, trajectories):
    ...
    print(
        f"  L antisymmetric: {torch.allclose(L + L.transpose(1, 2), torch.zeros_like(L))}"
    )
