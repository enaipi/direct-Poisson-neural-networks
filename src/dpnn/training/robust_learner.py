"""
RobustLearner: Unified learner for Hamiltonian/Poisson dynamical systems.
...
class RobustLearner:
    ...
    Usage:
        ...
        learner = RobustLearner(system_spec=spec, neurons=64)
    """
    ...
    def __init__(self,
        ...
        if system_spec is not None:
            ...
            print(f"RobustLearner initialized for {system_spec.name} (dim={self.dim})")
        else:
            self.system_spec = None
            self.model = model
            self.legacy_mode = True
            self.dim = self._get_legacy_dim(model, D)
            print(f"RobustLearner initialized for legacy model {model} (dim={self.dim})")
    ...
    def learn(self, method: str = "without", learning_rate: float = 1e-5,
        ...
        if method in ["without", "soft"]:
            ...
            optimizer = optim.Adam(
                list(self.energy.parameters()) + list(self.jac_vec.parameters()),
                lr=learning_rate
            )
